...
class Modify_Bloques:
    ...
    def generar_bloques_contexto(bloques):
        # Concatenamos todos los bloques en un solo texto
        texto = " ".join(bloques)

        # Convertimos el texto completo en una lista de palabras
        ...
        for bloque in bloques:
            # Convertimos el bloque en una lista de palabras
            palabras_bloque = bloque.split()

            # Buscamos la primera ocurrencia del bloque en el texto
            for i in range(len(palabras)):
                if palabras[i:i+len(palabras_bloque)] == palabras_bloque:
                    # Encontramos la posición del bloque
                    inicio = max(0, i - 5)  # Evitar índices negativos
                    fin = min(len(palabras), i + len(palabras_bloque) + 5)

                    # Tomamos las 5 palabras antes, el bloque y las 5 palabras después
                    contexto = palabras[inicio:fin]

                    # Añadimos el contexto a la lista
                    lista_contexto.append(" ".join(contexto))
                    break  # Solo queremos la primera ocurrencia del bloque
